...
Docker 构建需要拉取 Debian 系统依赖和 Playwright 浏览器，
国内网络不稳定时可指定镜像源：

```bash
# 方式一：docker build 直接传构建参数
docker build \
  ...
  --build-arg INSTALL_BROWSER=1 \
  -t parse-video-py .

# 方式二：docker compose（先设置环境变量，compose 会自动传入 build args）
...
说明：
- `APT_MIRROR`：替换容器内 Debian apt 源主机名，例如清华/阿里云镜像；
- `PLAYWRIGHT_DOWNLOAD_HOST`：Playwright 浏览器二进制下载地址，
  国内可用 npmmirror：`https://cdn.npmmirror.com/binaries/playwright`；
- `INSTALL_BROWSER`：默认 `0`（不安装无头浏览器，构建更快且不依赖 apt 网络）；
  设为 `1` 时才会安装 Chromium 及其系统依赖；
- 前两个参数均为可选，不设置时使用官方源。

> 容器默认不安装浏览器：抖音解析主路径是登录 Cookie 纯 HTTP，
> compose 中已设置 `PARSE_VIDEO_PY_DOUYIN_NO_BROWSER=1` 防止误用浏览器兜底。
> 只有在 `INSTALL_BROWSER=1` 构建时才需要移除该环境变量。
...
- 镜像：`baige778/parse-video-py:latest`
...
| 容器启动报 `TypeError: Server.__init__() takes 2 positional arguments but 3 were given` | `fastapi-mcp 0.4` 与 `mcp 2.x` 不兼容 | 已固定 `mcp>=1.0,<2.0`；升级 fastapi-mcp 后可放开 | `pyproject.toml` |
| 容器内解析任意平台（B站/抖音等）返回 500，异常为 `httpx.ConnectError`/`SSL: UNEXPECTED_EOF_WHILE_READING`，宿主机直连正常 | Docker/WSL2 虚拟网络问题（常见于 VPN/代理软件 TUN 模式导致 TLS 握手包被丢），脚本本身直连、不使用代理 | 尝试将 WSL2 虚拟网卡 MTU 调为 1350（管理员 PowerShell：`netsh interface ipv4 set subinterface "vEthernet (WSL)" mtu=1350 store=persistent`，然后 `wsl --shutdown` 并重启 Docker Desktop）；仍不行可在宿主机直接运行 uvicorn | `Dockerfile`、`docker-compose.yml` |
