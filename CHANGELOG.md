...
- **抖音动图（实况图）图集补全**：风控概率性返回精简 SSR 数据（图集图片不含 `video.play_addr`），原生路径曾把这种「缺动图视频地址」的图集当成功结果直接返回，导致动图只能解析出静图；现对「图集且所有图片均无 `live_photo_url`」的原生结果自动用浏览器兜底取完整数据补全，补全失败保持原生结果。普通视频与完整原生数据不触发，不影响常规解析速度
...
- **镜像版本标签补齐 `v` 前缀**：CI 此前仅推送无前缀版本号标签（如 `0.0.13`），与 README 中 `vX.Y.Z` 拉取示例不一致；现同时推送 `vX.Y.Z` 与 `X.Y.Z` 两种版本标签
...
- **GHCR 镜像自动关联仓库**：CI 构建时为镜像写入 `org.opencontainers.image.source` 标签与 index 注解（含镜像描述），GHCR 包自动关联到本仓库，仓库首页 Packages 面板可见
- **CI 支持版本号标签**：推送 `v*` git tag 时自动构建多架构（amd64/arm64）镜像并推送 `vX.Y.Z` / `vX.Y` 版本标签，补齐此前 `v0.0.12` 仅有单架构镜像的问题
...
- **解析失败报错精确化**：detail 接口未发起时区分「验证码拦截」与「视频观看限制」——页面出现安全验证/滑动等关键词时报验证码，否则报「视频存在观看限制（审核中/仅App可见/需登录等），暂无法解析」，不再把审核中/受限视频误报为验证码
- **Dockerfile 新增 PyPI 镜像**：直连 `files.pythonhosted.org` 在部分家宽环境会被断连，导致 `uv pip install` 重试失败、构建卡死；新增 `PYPI_MIRROR` 构建参数（默认清华源，经 `UV_DEFAULT_INDEX` 生效）
